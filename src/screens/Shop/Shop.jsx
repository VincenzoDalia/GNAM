  ...
  return (
    <>
      <CustomButton
        ...
        fontSize={"13px"}
      >
        <Text mb={1}>{props.coins < props.item.coins ? "Not enough money" : "Buy now"}</Text>
      </CustomButton>

      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent maxW="300px">
          <ModalHeader color="gray.600" fontSize="md" justifyContent={"center"}>You have selected</ModalHeader>

          <ModalBody>
            <Card maxW="xs" maxH={"xs"}>

              <Flex justifyContent="center" alignItems="center" bg={orangeLightColorBg}>
                <Image
              ...
              <Stack mt="6" spacing="1" alignItems="center">
                ...
                >
                  <Badge mb={3} bg={purpleColorButton} ml={1} pl={2} pr={2} color={whiteColorText} borderRadius={"10px"} fontWeight={"bold"}>
    ...
    <Card maxW="xs" maxH={"400px"} bg='white'>
      <CardBody>
        ...
        <Stack mt="6" spacing="1">
          <Heading fontSize={"15px"} color="#333333" >{props.item.name}</Heading>
          ...
          {isLocked ? (
            ...
            <Flex
              ...
              justifyContent={"center"}
            >
              <Badge mb={0} bg={purpleColorButton} ml={1} pl={2} pr={2} color={whiteColorText} borderRadius={"10px"} fontWeight={"bold"}>
                <Box display="flex" alignItems="center" gap={1}>
                  {props.item.coins} <PiCoinsFill color={whiteColorText} />
                </Box>
              </Badge>
        ...
        <Divider />
        {isLocked ? (
          <CustomButton isDisabled={coins < props.item.coins} bgColor={turquoiseColorExp}>Buy Now</CustomButton>
        ) : (
          <ConfirmationModal coins={coins} setCoins={setCoins} item={props.item} />
        )}
      </CardBody>
    </Card>
  );
...
const Shop = () => {
  ...
  useEffect(() => {
    API.getItems().then(res => {
      setItems(res.filter(item => item.bought == 0 && item.international == 0))
    })
  }, [])
